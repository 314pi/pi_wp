 ...
 */
function magicpi_uber_menu() {
	if ( ! get_theme_mod( 'magicpi_uber_menu', 1 ) ) {
		return;
	}
	$has_mobile_menu = has_nav_menu( 'primary_mobile' );
	$hide_for_medium = $has_mobile_menu ? 'hide-for-medium' : '';
	?>
	<div id="magicpi-uber-menu" class="header-ubermenu-nav relative <?php echo $hide_for_medium; ?>" style="z-index: 9">
		<div class="full-width">
			<?php ubermenu( 'main', array( 'theme_location' => 'primary' ) ); ?>
		</div>
	</div>
	<?php if ( $has_mobile_menu ) : ?>
		<div id="magicpi-uber-menu" class="header-ubermenu-nav relative show-for-medium" style="z-index: 9">
			<div class="full-width">
...
add_action( 'magicpi_after_header_bottom', 'magicpi_uber_menu', 10 );
