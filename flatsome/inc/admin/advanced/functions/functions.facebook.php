...
function magicpi_facebook_oauth_url() {
  $client_id = '380204239234502';
  $base_url = 'https://www.facebook.com/v4.0/dialog/oauth';
  $redirect_uri = magicpi_api_url() . '/facebook/authorize/';
  $scope = 'manage_pages,instagram_basic,public_profile';
  $nonce = wp_create_nonce( 'magicpi_facebook_connect_' . get_current_user_id() );
  $config = "optionsframework|of-option-instagram|$nonce";
  $return_uri = admin_url( "admin-ajax.php?magicpi_facebook_connect=$config" );
...
function magicpi_facebook_login_button_html() {
  $url = magicpi_facebook_oauth_url();
  ob_start(); ?>
  ...
  <a class="button" style="padding: 5px 15px; height: auto; background-color: #4267b2; border-color: #4267b2; color: #ffffff;" href="<?php echo $url ?>">
    ...
    <?php _e( 'Login with Facebook', 'magicpi-admin' ) ?>
  </a>
  <p>
    <a href="https://docs.uxthemes.com/article/379-how-to-connect-to-instagram-api" target="_blank" rel="noopener noreferrer">
      <?php _e( 'How to setup an Instagram Business account', 'magicpi-admin' ) ?>
    </a>
  </p>
  <?php return ob_get_clean();
}

function magicpi_facebook_accounts_html() {
...
function magicpi_facebook_set_theme_mod( $values, $old_values ) {
  $result = array();
...
add_filter( 'pre_set_theme_mod_facebook_accounts', 'magicpi_facebook_set_theme_mod', 10, 2 );

function magicpi_facebook_connect() {
  $action = 'magicpi_facebook_connect_' . get_current_user_id();
...
add_action( 'wp_ajax_magicpi_facebook_connect', 'magicpi_facebook_connect' );
