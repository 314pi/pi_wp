<?php
function magicpi_checkout_breadcrumb_class( $endpoint ) {
	$classes = array();
...
<nav class="breadcrumbs flex-row flex-row-start checkout-breadcrumbs text-left medium-text-center is-large <?php echo get_theme_mod( 'cart_steps_case', 'uppercase' ); ?>">
	...
	<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="<?php echo magicpi_checkout_breadcrumb_class( 'cart' ); ?>">
		<?php if ( $steps ) echo '<span class="breadcrumb-step hide-for-small">1</span>'; ?>
		<?php esc_html_e( 'Shopping Cart', 'magicpi' ); ?>
	</a>
	<span class="divider hide-for-small"><?php echo get_magicpi_icon( 'icon-angle-right' ); ?></span>
	<a href="<?php echo esc_url( wc_get_checkout_url() ); ?>" class="<?php echo magicpi_checkout_breadcrumb_class( 'checkout' ); ?>">
		<?php if ( $steps ) echo '<span class="breadcrumb-step hide-for-small">2</span>'; ?>
		<?php esc_html_e( 'Checkout details', 'magicpi' ); ?>
	</a>
	<span class="divider hide-for-small"><?php echo get_magicpi_icon( 'icon-angle-right' ); ?></span>
	<a href="#" class="no-click <?php echo magicpi_checkout_breadcrumb_class( 'order-received' ); ?>">
		<?php if ( $steps ) echo '<span class="breadcrumb-step hide-for-small">3</span>'; ?>
		<?php esc_html_e( 'Order Complete', 'magicpi' ); ?>
	</a>
</nav>
