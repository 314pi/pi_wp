 ...
 *
 * @author  UX Themes
 * @package Magicpi/WooCommerce
 */

if ( ! function_exists( 'magicpi_sale_flash' ) ) {
	/**
	 ...
	 * @return string
	 */
	function magicpi_sale_flash( $text, $post, $_product, $badge_style ) {
		global $wc_cpdf;
		...
		if ( $wc_cpdf->get_value( get_the_ID(), '_bubble_new' ) ) {
			...
			$bubble_text = $wc_cpdf->get_value( get_the_ID(), '_bubble_text' ) ? $wc_cpdf->get_value( get_the_ID(), '_bubble_text' ) : __( 'New', 'magicpi' );
...
add_filter( 'magicpi_product_labels', 'magicpi_sale_flash', 10, 4 );


if ( ! function_exists( 'magicpi_woocommerce_get_alt_product_thumbnail' ) ) {
	/**
	 * Get Hover image for WooCommerce Grid
	 */
	function magicpi_woocommerce_get_alt_product_thumbnail() {
		...
		if ( $attachment_ids ) {
			...
			foreach ( $attachment_ids as $attachment_id ) {
				...
				$loop ++;
				echo apply_filters( 'magicpi_woocommerce_get_alt_product_thumbnail',
...
add_action( 'magicpi_woocommerce_shop_loop_images', 'magicpi_woocommerce_get_alt_product_thumbnail', 11 );
...
if ( ! function_exists( 'magicpi_woocommerce_shop_loop_category' ) ) {
	/**
	 * Add and/or Remove Categories
	 */
	function magicpi_woocommerce_shop_loop_category() {
		if ( ! magicpi_option( 'product_box_category' ) ) {
			return;
		} ?>
		<p class="category uppercase is-smaller no-text-overflow product-cat op-7">
			...
			if ( $product_cats ) {
				...
				echo esc_html( apply_filters( 'magicpi_woocommerce_shop_loop_category', $first_part, $product ) );
			}
			?>
		</p>
	<?php
...
add_action( 'woocommerce_shop_loop_item_title', 'magicpi_woocommerce_shop_loop_category', 0 );

if ( ! function_exists( 'magicpi_woocommerce_shop_loop_ratings' ) ) {
	/**
	 * Add and/or Remove Ratings
	 */
	function magicpi_woocommerce_shop_loop_ratings() {
		// Switch ratings position when grid 3 is chosen.
		if ( 'grid3' === magicpi_option( 'grid_style' ) && ! get_theme_mod( 'disable_reviews' ) ) {
			remove_action( 'woocommerce_after_shop_loop_item_title', 'woocommerce_template_loop_rating', 5 );
			add_action( 'woocommerce_shop_loop_item_title', 'woocommerce_template_loop_rating' );
		}
		// Remove ratings.
		if ( magicpi_option( 'product_box_rating' ) ) {
			return;
		}
		remove_action( 'woocommerce_after_shop_loop_item_title', 'woocommerce_template_loop_rating', 5 );
...
add_action( 'woocommerce_shop_loop_item_title', 'magicpi_woocommerce_shop_loop_ratings', 0 );


/* Remove and add Product image */
remove_action( 'woocommerce_before_shop_loop_item_title', 'woocommerce_template_loop_product_thumbnail', 10 );
add_action( 'magicpi_woocommerce_shop_loop_images', 'woocommerce_template_loop_product_thumbnail', 10 );

/* Remove Default Add To cart button from Grid */
remove_action( 'woocommerce_after_shop_loop_item', 'woocommerce_template_loop_add_to_cart', 10 );

if ( ! function_exists( 'magicpi_product_box_actions_add_to_cart' ) ) {
	/**
...
add_action( 'magicpi_product_box_actions', 'magicpi_product_box_actions_add_to_cart', 1 );

if ( ! function_exists( 'magicpi_woocommerce_shop_loop_button' ) ) {
	/**
...
add_action( 'magicpi_product_box_after', 'magicpi_woocommerce_shop_loop_button', 100 );
 ...
 * @return array
 */
function magicpi_woocommerce_loop_add_to_cart_args( $args, $product ) {

	if ( ! doing_action( 'magicpi_product_box_actions' ) && ! doing_action( 'magicpi_product_box_after' ) ) {
		return $args;
	}
...
add_filter( 'woocommerce_loop_add_to_cart_args', 'magicpi_woocommerce_loop_add_to_cart_args', 5, 2 );
 ...
 * @return string
 */
function magicpi_woocommerce_loop_add_to_cart_link( $link, $product, $args ) {
...
add_filter( 'woocommerce_loop_add_to_cart_link', 'magicpi_woocommerce_loop_add_to_cart_link', 5, 3 );

if ( ! function_exists( 'magicpi_woocommerce_shop_loop_excerpt' ) ) {
	/**
	 * Add Product Short description
	 */
	function magicpi_woocommerce_shop_loop_excerpt() {
		if ( ! magicpi_option( 'short_description_in_grid' ) ) {
			return;
		}
		?>
...
add_action( 'magicpi_product_box_after', 'magicpi_woocommerce_shop_loop_excerpt', 20 );

if ( ! function_exists( 'magicpi_product_box_class' ) ) {
	/**
	 * Add Classes to product box
	 *
	 * @return null/string
	 */
	function magicpi_product_box_class() {
		$classes             = array();
...
if ( ! function_exists( 'magicpi_product_box_image_class' ) ) {
	/**
	 * Add Classes to product image box
	 *
	 * @return null/string
	 */
	function magicpi_product_box_image_class() {
		$hover_style = magicpi_option( 'product_hover' );
		if ( $hover_style == 'fade_in_back' && $hover_style == 'zoom_in' ) {
			return null;
		}
		$classes   = array();
...
if ( ! function_exists( 'magicpi_product_box_actions_class' ) ) {
	/**
	 * Add Classes to product actions
	 *
	 * @return string
	 */
	function magicpi_product_box_actions_class() {
		return 'grid-tools text-center hide-for-small bottom hover-slide-in show-on-hover';
	}
}

if ( ! function_exists( 'magicpi_product_box_text_class' ) ) {
	/**
	 * Add classes to product text box
	 *
	 * @return string
	 */
	function magicpi_product_box_text_class() {
		$classes = array( 'box-text-products' );

		$grid_style = magicpi_option( 'grid_style' );
