 ...
 *
 * @author UX Themes
 * @package Magicpi/Shortcodes/Accordion
 * @version 3.9.0
 */
...
function ux_accordion( $atts, $content = null ) {
	...
	return $title . '<div class="' . implode( ' ', $classes ) . '" rel="' . $open . '">' . magicpi_contentfix( $content ) . '</div>';
}
...
function ux_accordion_item( $atts, $content = null ) {
	...
	if ( $class ) $classes[] = $class;
	return '<div class="' . implode( ' ', $classes ) . '"><a href="#" class="accordion-title plain"><button class="toggle"><i class="icon-angle-down"></i></button><span>' . $title . '</span></a><div class="accordion-inner">' . magicpi_contentfix( $content ) . '</div></div>';
}
