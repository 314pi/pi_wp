...
<?php
  ...
  if(magicpi_option('catalog_mode')) { get_template_part('template-parts/header/partials/element','cart-replace'); return;}
  $cart_style = magicpi_option('header_cart_style');
  $custom_cart_content = magicpi_option('html_cart_header');
  $icon_style = magicpi_option('cart_icon_style');
  $icon = magicpi_option('cart_icon');
...
<?php
if(magicpi_option('custom_cart_icon')) { ?>
  <span class="image-icon header-cart-icon" data-icon-label="<?php echo WC()->cart->cart_contents_count; ?>">
    <img class="cart-img-icon" alt="<?php _e('Cart', 'woocommerce'); ?>" src="<?php echo do_shortcode(magicpi_option('custom_cart_icon')); ?>"/>
  </span>
<?php }
else { ?>
  <?php if(!$icon_style) { ?>
